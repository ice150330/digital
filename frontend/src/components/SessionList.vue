...
<style scoped>
.session-list { display: flex; min-height: 480px; height: min(720px, calc(100vh - 180px)); min-width: 0; flex-direction: column; gap: var(--space-3); overflow: hidden; padding: var(--space-3); background: var(--bg-card); border: 1px solid var(--border-default); border-radius: var(--radius-card); }
...
.session-search:focus-within { border-color: var(--border-focus); box-shadow: var(--ring-focus); }
...
.runtime-pill { display: inline-flex; max-width: 76px; min-height: var(--tag-height-sm); flex: 0 0 auto; align-items: center; padding: 0 var(--space-2); overflow: hidden; border-radius: var(--radius-sm); background: var(--bg-subtle); color: var(--text-secondary); font-size: var(--font-size-xs); line-height: var(--tag-height-sm); text-overflow: ellipsis; white-space: nowrap; }
